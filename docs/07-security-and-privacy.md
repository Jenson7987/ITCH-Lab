...
| 7. Dependency/secrets | `pip-audit` found no known vulnerability in the hashed release lock. `detect-secrets` found zero real secrets; 18 reviewed entropy findings are fixed hashes and partition/path test literals recorded as false positives in the baseline. |
